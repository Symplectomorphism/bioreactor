...
To solve this circuit, assume an ideal op-amp model so that the inputs of the
op-amp both has potential $v_i$. Let the current $i$ flowing over $R_{d1}$ split
into $i_1$ and $i_2$, the former flowing into $C_{d1}$ and the latter into
...
\end{align*}
%
KVL around the bottom loop gives $v_m = R_{d2}i_2 + v_i = R_{d2}C_{d2}\frac{\dd
v_i}{\dd t} + v_i$. Plugging this into the second equation gives $i_1 =
R_{d2}C_{d1}C_{d2}\frac{\dd^2v_i}{\dd t^2}$. The first and third
equations above combine with this expression for $i_1$ to yield
%
\begin{equation}
