...
Theory and simulation indicate that that some good values for the resistances
$R_1$, $R_2$ and the capacitances $C_1$, $C_2$ that obey the constraints are
$C_2 = 1$\unit{\nano\farad}. Then from the inequality above, we must have $R_1 +
R_2 \leq 500$\unit{\kilo\ohm}. We distribute this according to $R_1 = 4R_2$ so
with $R_2 = 100$\unit{\kilo\ohm}. Finally, using the equality~\eqref{eq:cons1}
with these resistances, we obtain $C_2 = \nicefrac{25}{8}C_1 =
3.125$\unit{\nano\farad}.
