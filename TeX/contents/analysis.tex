...
Sallen-Key low-pass filter as shown in Figure~\ref{fig:sallenkey}.

\begin{figure}[h]
\begin{circuitikz}[scale=1]\draw
(5,.5) node [op amp] (opamp) {}
(0,0) node [left] {$v_f$} to [R, l=$R_d$, o-*] (2,0)
to [R, l=$R_d$, *-*] (opamp.+)
...
Section~\ref{ssec:second}. Simulation shows that some good values for the
resistances $R_d$ and the capacitances $C_d$ are $R_d = 68$\unit{\kilo\ohm} and
$C_d = 1$\unit{\nano\farad}, with a cutoff frequency of $f \approx
2340$\unit{\hertz}. Even $R_d = 100$\unit{\kilo\ohm} works well with $C_d =
1$\unit{\nano\farad} with a cutoff frequency at $f \approx 1591$\unit{\hertz}.
